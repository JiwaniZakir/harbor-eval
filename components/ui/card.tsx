...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-[var(--radius-lg)] bg-[var(--bg-card)] shadow-[var(--shadow-card)] transition-shadow duration-[var(--duration-normal)]",
        className,
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("text-sm font-semibold leading-tight text-[var(--foreground)]", className)}
      {...props}
    />
  ),
);
